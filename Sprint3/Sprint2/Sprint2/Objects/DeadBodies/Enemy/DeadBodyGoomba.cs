...
{
    class DeadBodyGoomba : IObject
    {
        double yPos;
        double xPos;
        double xVelos;
        Texture2D texture;
        Rectangle refRectangle = new Rectangle(Rec.DEADBODYGOOMBA_X, Rec.DEADBODYGOOMBA_Y, Rec.DEADBODYGOOMBA_W, Rec.DEADBODYGOOMBA_H);
        float rotation = (float)Math.PI;
        int deathBumpCounter = 16;

        public DeadBodyGoomba(int x, int y, Texture2D texture, CollisionType.Collision c)
        {
            yPos = y;
            xPos = x;
