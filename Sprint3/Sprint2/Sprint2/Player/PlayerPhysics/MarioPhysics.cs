...
namespace Sprint2.Player.PlayerPhysics
{
    class MarioPhysics
    {
        ...
        public MarioPhysics(Mario p, int x, int y)
        {
            mario = p;
            xPos = x;
            yPos = y;
        ...
        public void Update()
        {
            ...
            if ((!(( (isPressingUp && !wasPressingUp) || isPressingDown) || (isPressingLeft || isPressingRight))) && !isJumping) 
                mario.Idle();

            if (mario.ToString().Equals("dead"))
            {
                xVelosMax = 0;
                xVelosMin = 0;
                yVelosMax = 0;
